...
data_type='PA' # Type of data from ASVSpoof challenge datasets
augment_type='AddBackgroundNoise'
...
if not os.path.exists(augment_type): # creating Directory with augment type name
...
with open('/scratch/projects/Datasets/ASVSpoof2019/'+data_type+'/ASVspoof2019_'+data_type+'_cm_protocols/ASVspoof2019.'+data_type+'.cm.train.trn.txt', 'r') as f:
    for i,line in enumerate(tqdm(f)): #train_raw_labels
        ...
        if str(t_lab)=='spoof':
            ...
            augment = Compose([AddBackgroundNoise()])#Path, str, A path or list of paths to audio file(s) and/or folder(s) with audio files. Can be str or Path instance(s).
