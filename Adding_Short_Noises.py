...
from os import path
from audiomentations import AddShortNoises 
import acoustics
...
# Adding the short noise to the audio this augmentation required the noise file to merge in the audio


data_type='PA'
augment_type='AddShortNoises'
...
with open('/scratch/projects/Datasets/ASVSpoof2019/'+data_type+'/ASVspoof2019_'+data_type+'_cm_protocols/ASVspoof2019.'+data_type+'.cm.train.trn.txt', 'r') as f:
    for i,line in enumerate(tqdm(f)): #train_raw_labels
        ...
        if str(t_lab)=='spoof':
            ...
            augment = Compose([AddShortNoises()])#Path, str, A path or list of paths to audio file(s) and/or folder(s) with audio files. Can be str or Path instance(s).
